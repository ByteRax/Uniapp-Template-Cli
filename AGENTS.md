<!-- Generated: 2026-03-28 | Updated: 2026-05-11 -->

# Uniapp-Template-Cli

## Purpose

基于 UniApp + Vue 3 + TypeScript + Vite 5 + UnoCSS 的多端模板工程。仓库以配置驱动方式生成页面与 manifest，入口在 `src/main.ts`，启动时按 `setupStore()`、`setupRoute()`、`setupHttp()` 顺序挂载状态、路由和请求层，并在入口处配置 `z-paging` 全局默认行为。

## Key Files

| File                   | Description                                              |
|------------------------|----------------------------------------------------------|
| `package.json`         | 项目依赖、端侧脚本、版本号、lint-staged 与包管理器约束。                          |
...
| `eslint.config.mjs`    | ESLint 9 扁平配置，结合 `@uni-helper/eslint-config` 与 UnoCSS 插件。     |
| `.prettierrc.cjs`      | Prettier 配置，主要由 lint-staged 和 format 脚本消费。                    |
...
| `docs/`         | 项目工程化说明、方案文档和 superpowers/specs 历史设计稿。             |
| `.agent/`       | AI 协作文档与历史笔记（见 `.agent/AGENTS.md`）。           |
| `.github/`      | GitHub/Copilot 侧的协作说明（见 `.github/AGENTS.md`）。 |
| `.husky/`       | Git 提交钩子脚本（见 `.husky/AGENTS.md`）。             |
| `.rules/`       | 项目规则补充文档，作为辅助参考，事实以当前代码和根级协作文档为准。              |
| `.omc/`         | Oh My ClaudeCode/本地会话状态与调研草稿，不参与运行时。                |
...
- 端侧命令、依赖版本和脚本名称以 `package.json` 为准；README 中仍可能保留 `pnpm dev`、`pnpm dev:mp`、`pnpm build:mp` 等旧写法。
- 自动导入覆盖面很广，重命名 `src/composables`、`src/stores`、`src/utils`、`src/router` 中的导出前先全局检索引用。
- 不要把 `.omc/`、`dist/`、`node_modules/`、生成类型文件或 IDE 状态当作需要维护的源码事实。
...
- 文档-only 改动至少核对引用路径、命令和生成产物规则是否仍与当前仓库一致。
...
- 页面通过 `<route lang="json">` 块和 `@uni-helper/vite-plugin-uni-pages` 进入页面配置；主包在 `src/pages/`，分包在 `src/pages-sub/`。
- `src/services/platformService.ts` 承载跨端平台服务封装，当前包含微信 API 适配与地图导航。
- UnoCSS 同时使用 `presetUni`、图标预设、暗色颜色快捷方式与 `size-*` / safe-area 等动态规则。
...
- `src/router/`、`src/http/`、`src/stores/` 共同依赖登录态、页面元数据和统一 UI 状态。
...
- `@uni-helper/*` / `@uni-ku/*` - 页面、manifest、组件自动生成和分包优化能力。
